...
| Proxy credentials                         | Plaintext — `sessions.proxyUrl` may embed `user:pass`                         | Database access control; never returned by the session read DTOs — only masked `proxyHost` and `hasCredentials` on `GET/PATCH /proxy`, and `proxyEnabled`/`proxyType`/`proxyHost` on session list/get. |
